...
function raiseDispute(uint256 commissionId) external {
    ...
    require(commission.collector != address(0), "commission does not exist");
    require(
        msg.sender == commission.collector || msg.sender == commission.artisan,
...
function resolveDispute(uint256 commissionId, bool payArtisan)
    external
    nonReentrant
{
    Commission storage commission = commissions[commissionId];
    ...
    if (payArtisan) {
        ...
        (bool success, ) =
            payable(commission.artisan).call{value: amount}("");
        require(success, "payment failed");
    } else {
        commission.status = Status.Refunded;

        (bool success, ) =
            payable(commission.collector).call{value: amount}("");
        require(success, "refund failed");
    }
}
}